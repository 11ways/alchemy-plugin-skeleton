...
const McpManager = Function.inherits('Thoth.Mcp.Base', 'Thoth.Mcp', function Manager(server) {
	...
	// Pending upload slots, keyed by their secret token. The anonymous upload
	// endpoint (which carries no session) resolves a slot by this token. The
	// same slot object is also indexed by its readable reference on the
	// owning session (see McpSession#upload_slots).
	this.upload_slots = new Map();
});

/**
 * How long an upload slot stays valid after creation (15 minutes).
 *
 * @author   Jelle De Loecker   <jelle@elevenways.be>
 * @since    0.2.2
 */
...
/**
 * Create a new upload slot for a session.
 *
 * Returns a slot with two identifiers:
 *  - `token`: a secret, unguessable value that authorises the (sessionless)
 *    HTTP upload. It is the only credential the uploading `curl` carries.
 *  - `reference`: a short, readable, session-scoped id (U001, U002, ...) the
 *    LLM passes back into a tool to consume the uploaded file.
 *
 * @author   Jelle De Loecker   <jelle@elevenways.be>
 * @since    0.2.2
 *
 * @param    {Thoth.Mcp.Session}   session
 * @param    {Object}              options
 * @param    {string}             [options.filename]   Suggested final filename
 *
 * @return   {Object}   The created slot
 */
McpManager.setMethod(function createUploadSlot(session, options = {}) {

	if (!session) {
		throw new Error('Cannot create an upload slot without a session');
	}

	let now = Date.now();
	let token = Blast.Classes.Crypto.randomHex(12);
	let reference = 'U' + String(++session._upload_seq).padStart(3, '0');
	...
	this.upload_slots.set(token, slot);
	session.upload_slots.set(reference, slot);

	return slot;
});

/**
 * Resolve an upload slot by its secret token (used by the HTTP upload
 * endpoint, which has no session context). Returns null when missing or
 * expired.
 *
 * @author   Jelle De Loecker   <jelle@elevenways.be>
 * @since    0.2.2
 ...
 * @return   {Object|null}
 */
McpManager.setMethod(function getUploadSlot(token) {

	if (!token) {
		return null;
	}

	let slot = this.upload_slots.get(token);
...
/**
 * Record an uploaded file on a slot. The temp file is moved into a thoth-owned
 * temp directory so its lifetime is under our control until consumed/expired.
 *
 * @author   Jelle De Loecker   <jelle@elevenways.be>
 * @since    0.2.2
 *
 * @param    {Object}   slot   The slot (from getUploadSlot)
 * @param    {Object}   file   The uploaded file (conduit.files.file)
 *
 * @return   {Promise<Object>}   The updated slot
 */
McpManager.setMethod(async function storeUpload(slot, file) {

	const libpath = alchemy.use('path');
	const fs = alchemy.use('fs');

	// Uploaded files arrive as Alchemy Inode.File objects (.path/.name, with the
	// client-declared type in .possible_type since .type is withheld as
	// untrusted), but also support a plain {path, name, type} shape.
	let source_path = file.path || file.filepath;
	let original_name = file.name || file.originalFilename || null;
	let mimetype = file.type || file.possible_type || file.mimetype || null;

	if (!mimetype && typeof file.getMimetype === 'function') {
		mimetype = await file.getMimetype().catch(() => null);
	}

	let dir = libpath.join(PATH_TEMP, 'mcp_uploads');
	let target = libpath.join(dir, slot.token);

	await fs.promises.mkdir(dir, {recursive: true});

	// rename() fails across devices (the multipart temp dir may live on a
	// different mount than PATH_TEMP), so copy + unlink the source.
	await fs.promises.copyFile(source_path, target);
	await fs.promises.unlink(source_path).catch(() => {});

	let stat = await fs.promises.stat(target);

	slot.temp_path = target;
	slot.original_filename = original_name;
...
/**
 * Consume an upload slot by its readable reference within a session.
 *
 * Single-use and session-scoped: the reference is looked up on the session,
 * so one session can never consume another's slot. Throws a clean McpError
 * when the reference is unknown, expired, not yet uploaded, or already used.
 *
 * @author   Jelle De Loecker   <jelle@elevenways.be>
 * @since    0.2.2
 *
 ...
 *
 * @return   {Object}   {temp_path, filename, mimetype, size}
 */
McpManager.setMethod(function consumeUploadSlot(session, reference) {

	const McpError = Classes.Thoth.Mcp.Error;

	if (!session) {
		throw new McpError('No MCP session available to consume an upload');
	}

	let slot = session.upload_slots.get(reference);

	if (!slot) {
		throw new McpError('Unknown upload reference "' + reference + '". Call request_upload_path first.');
	}

	if (Date.now() > slot.expires_at) {
		this.deleteUploadSlot(slot);
		throw new McpError('Upload reference "' + reference + '" has expired. Request a new upload path.');
	}

	if (!slot.uploaded) {
		throw new McpError('Nothing has been uploaded to "' + reference + '" yet.');
	}

	if (slot.used) {
		throw new McpError('Upload reference "' + reference + '" has already been used.');
	}
	...
	return {
		...
		filename  : slot.filename || slot.original_filename || null,
...
/**
 * Remove an upload slot from both indexes and unlink its temp file.
 *
 * @author   Jelle De Loecker   <jelle@elevenways.be>
 * @since    0.2.2
 *
 * @param    {Object}   slot
 */
McpManager.setMethod(function deleteUploadSlot(slot) {

	if (!slot) {
		return;
	}

	this.upload_slots.delete(slot.token);
	slot.session?.upload_slots?.delete(slot.reference);

	if (slot.temp_path) {
		const fs = alchemy.use('fs');
		fs.promises.unlink(slot.temp_path).catch(() => {});
	}
});

/**
 * Clean up expired upload slots (and their temp files).
 *
 * @author   Jelle De Loecker   <jelle@elevenways.be>
 * @since    0.2.2
 */
McpManager.setMethod(function cleanupUploadSlots() {

	let now = Date.now();

	for (let [, slot] of this.upload_slots) {
