...
McpServer.setMethod(function createRoutes() {
	...
	// Deliberately a plain route (NOT the MCP conduit class) so Alchemy parses
	// the multipart body and populates `conduit.files`. The secret token in the
	// path is the only credential needed - it authorises a sessionless upload.
	Router.add({
		name             : 'Mcp@' + server_name + '#upload',
		handler          : 'ThothMcp#upload',
		methods          : 'post',
		paths            : path + '/upload/{token}',
